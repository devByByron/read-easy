...
const LANGUAGE_MODELS = [
	...
	},
	{
		...
		name: "French",
		model: "French",
		voiceLangs: ["fr-FR", "fr-CA", "fr-BE", "fr-CH", "fr"],
	},
	{
		code: "es",
		name: "Spanish",
		model: "Spanish",
		voiceLangs: ["es-ES", "es-MX", "es-AR", "es-CO", "es-US", "es"],
	},
	{
		code: "de",
		name: "German",
		model: "German",
		voiceLangs: ["de-DE", "de-AT", "de-CH", "de"],
	},
	{
		code: "it",
		name: "Italian",
		model: "Italian",
		voiceLangs: ["it-IT", "it-CH", "it"],
	},
	{
		code: "zh",
		name: "Chinese",
		model: "Chinese",
		voiceLangs: ["zh-CN", "zh-TW", "zh-HK", "zh"],
	},
	{
		code: "ar",
		name: "Arabic",
		model: "Arabic",
		voiceLangs: ["ar-SA", "ar-AE", "ar-EG", "ar-MA", "ar"],
	},
	{
		code: "ru",
		name: "Russian",
...
const TextProcessor = ({ extractedText, fileName }: TextProcessorProps) => {
	...
	const findVoiceForLanguage = (langCode: string, preferLocal = true) => {
		...
		for (const voiceLang of langModel.voiceLangs) {
			// Try exact match
			const exactMatch = finalVoices.find(
				(voice) => voice.lang === voiceLang
			);
			if (exactMatch) {
				console.log(
					`Found exact voice match for ${langCode}:`,
					exactMatch.name,
					exactMatch.lang,
					"Reliable:",
					exactMatch.localService !== false
				);
				return exactMatch;
			}

			// Try partial match (e.g., 'fr' matches 'fr-FR')
			const partialMatch = finalVoices.find((voice) =>
				voice.lang.startsWith(voiceLang)
			);
			if (partialMatch) {
				console.log(
					`Found partial voice match for ${langCode}:`,
					partialMatch.name,
					partialMatch.lang,
					"Reliable:",
					partialMatch.localService !== false
				);
				return partialMatch;
			}
		}
	...
	const handleLanguageChange = (newLanguage: string) => {
		...
		if (autoSwitchVoice && voices.length > 0) {
			...
			if (languageVoice) {
				...
				console.log(
					`Auto-switched voice for ${newLanguage}:`,
					languageVoice.name
				);
				toast({
					title: "Voice Auto-Selected",
					description: `Switched to ${languageVoice.name} for ${
						LANGUAGE_MODELS.find((l) => l.code === newLanguage)
							?.name || newLanguage
					}`,
				});
			}
	...
	const filterWorkingVoices = (
		...
		return voices.filter((voice) => {
			...
			if (voice.localService === false) {
				...
				if (!isTrustedRemote) {
					console.log(
						"Filtered out untrusted remote voice:",
						voice.name
					);
					return false;
				}
	...
	useEffect(() => {
		...
		if (!isSupported) {
			...
			toast({
				...
				description:
					"Text-to-speech requires a secure HTTPS connection to work properly.",
				variant: "destructive",
			});
		} else {
	...
	useEffect(() => {
		...
		const loadVoices = async () => {
			...
			console.log(
				...
				availableVoices.map(
					(v) => `${v.name} (${v.lang}) [Local: ${v.localService}]`
				)
			);
			...
			if (availableVoices.length > 0) {
				...
				console.log(
					`Filtered voices: ${filteredVoices.length}/${availableVoices.length} voices remaining`
				);
				...
				if (!selectedVoice) {
					...
					const systemVoices = availableVoices.filter(
						(voice) =>
							!voice.name.includes("Google") &&
							!voice.name.includes("Remote")
					);
					const allVoices =
		...
		const timeouts = [100, 500, 1000, 2000].map((delay, index) =>
			setTimeout(() => {
				if (voices.length === 0) {
					console.log(
						`Fallback voice loading attempt ${index + 1}...`
					);
					loadVoices();
				}
			}, delay)
		...
		if (isPlaying && utteranceRef.current) {
			try {
				...
				utterance.onerror = (event) => {
					console.error(
						"Speech error during playback update:",
						event
					);
					setIsPlaying(false);
					...
					if (currentVoice) {
						...
						if (nextVoice) {
							console.log(
								"Auto-switching to next reliable voice:",
								nextVoice.name
							);
	...
	const getValidVoice = () => {
		...
		if (selectedVoice) {
			...
			console.log(
				"Selected voice not found in filtered list:",
				selectedVoice
			);
			...
			if (replacementVoice) {
				console.log(
					"Auto-selecting replacement voice:",
					replacementVoice.name
				);
		...
		if (processedText && activeProcessor === "translate") {
			...
			console.log(
				`Looking for voice for translated text in: ${targetLanguage}`
			);
			...
			if (languageVoice) {
				console.log(
					"Found voice for translated text:",
					languageVoice.name,
					languageVoice.lang
				);
				setSelectedVoice(languageVoice.name);
		...
		const fallbackVoice =
			languageVoice || englishVoice || voicesToSearch[0] || voices[0];
	...
	const handlePlay = () => {
		...
		try {
			...
			utterance.onerror = (event) => {
				...
				if (currentVoice) {
					console.log(
						"Speech failed with voice:",
						currentVoice.name,
						"Finding alternative..."
					);

					// Find the next best voice (excluding the failed one)
					const alternativeVoice = voices.find(
						(v) =>
							v.name !== currentVoice.name &&
							v.localService !== false
					);
				...
				toast({
					title: "Speech Error",
					description:
						"There was an error with text-to-speech. All voices have been tested for compatibility.",
					variant: "destructive",
				});
			};
	...
	const processWithAI = async (type: string) => {
		...
		try {
			...
			let langName = "";
			if (type === "translate") {
				const lang = LANGUAGE_MODELS.find(
			...
			let errorMessage =
				error.message || "There was an error processing the text.";
			...
			if (
				...
				errorMessage =
					"Text is too long for processing. Try summarizing first, then translate the summary.";
			} else if (
				error.message?.includes("Function error: Gateway Timeout")
			) {
				errorTitle = "Processing Timeout";
				errorMessage =
					"The request took too long. Please try with shorter text or summarize first.";
			}
						...
						{!speechSupported ? (
							<div className="p-4 bg-muted rounded-lg">
								<p className="text-sm text-muted-foreground">
									Text-to-speech is not available in your
									browser. Please try using Chrome, Firefox,
									or Safari.
								</p>
							</div>
						) : (
									...
									>
										...
										<SelectContent>
											{(() => {
												...
												return sortedLangs.flatMap(
													(langCode) => {
														const langName =
															LANGUAGE_MODELS.find(
																(l) =>
																	l.code ===
																	langCode
															)?.name ||
					...
					<div className="flex items-center justify-between mb-4">
						...
						<div className="flex items-center gap-3">
							<span className="text-sm text-muted-foreground">
								{(processedText || extractedText).length}{" "}
								characters
								{(processedText || extractedText).length >
									3000 && (
